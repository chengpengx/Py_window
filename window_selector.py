...
#                print('win_obj.dt',win_obj.dt)
#                print('self.observed.stats.delta',self.observed.stats.delta)
#                print('#########################################')
#                print('self.observed.stats.starttime',self.observed.stats.starttime)
            if abs(win_obj.time_of_first_sample -
                    self.observed.stats.starttime) > \
                    1 * self.observed.stats.delta:
                    #0.5 * self.observed.stats.delta:(xichengpeng修改)
                raise PyflexError(
...
#        print('self.stalta',self.stalta)
####################### Original #########################
###################### Jiang change #########################
#        logger.info("Calculating envelope of observed.")
#        self.observed_envelope = envelope(self.observed.data)
#        logger.info("Calculating STA/LTA.")
#        self.stalta = sta_lta(self.observed_envelope,
#                              self.observed.stats.delta,
#                              self.config.main_period)
###################### Jiang change #########################
        ...
        if self.ttimes:
            ...
            # print('%%%%%%%%%%%%self.event.origin_time%%%%%%%%%%%%',self.event.origin_time)
            # print('self.ttimes[0]["time"]',self.ttimes[0]["time"])
            # print('self.config.max_time_before_first_arrival',self.config.max_time_before_first_arrival)
            # print('offset',offset)
...
#            max_time = dist_in_km / 0.03 + offset + self.config.max_period
            ...
            # print('self.observed.stats.delta',self.observed.stats.delta)
            # print('self.config.min_surface_wave_velocity',self.config.min_surface_wave_velocity)
            # print('self.config.max_period',self.config.max_period)
            ...
            # print('self.troughs[0]',self.troughs[0])
            # print('self.troughs[-1]',self.troughs[-1])
            min_trough, max_trough = self.troughs[0], self.troughs[-1]
            self.peaks = self.peaks[(self.peaks > min_trough) &
                                    (self.peaks < max_trough)]
            ####################xichengpeng###########################
            # print('min_trough',min_trough)
            # print('max_trough',max_trough)
    ...
    def reject_based_on_signal_to_noise_ratio(self):
        ...
        if self.config.window_signal_to_noise_type == "amplitude":
            ...
            def filter_window_noise(win):
                ...
                print('win_energy',win_energy,'noise_energy',noise_energy)
                if win_noise_amp < self.config.s2n_limit[win.center]:
                    return False
                return True
        else:
...
#            win._calc_criteria(self.observed.data, self.synthetic.data)
        ...
        def reject_based_on_criteria(win):
            ...
            dlnA_max = self.config.dlna_reference + \
                self.config.dlna_acceptance_level[win.center]
#            print('self.config.dlna_reference',self.config.dlna_reference)
#            print('self.config.dlna_acceptance_level[win.center]',
#                  self.config.dlna_acceptance_level[win.center])
            # print('tshift_min','win.cc_shift','self.observed.stats.delta','tshift_max',tshift_min,win.cc_shift,self.observed.stats.delta,tshift_max)
            if not (tshift_min < win.cc_shift *
                    self.observed.stats.delta < tshift_max):
                logger.debug("Window rejected due to time shift: %f" %
                             win.cc_shift)
                return False
#            print('dlnA_min',dlnA_min,'win.dlnA',win.dlnA,'dlnA_max',dlnA_max)
